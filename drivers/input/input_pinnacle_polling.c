...
#define DT_DRV_COMPAT cirque_pinnacle_polling
...
static int pinnacle_poll(const struct device *dev)
{
	const struct pinnacle_config *config = dev->config;
	...
	/* Match QMK's polling path: packets are valid only while SW_DR is set. */
	if ((status & PINNACLE_STATUS1_SW_DR) == 0U) {
		return 0;
	}
	...
	if (config->relative_mode) {
		input_report_rel(dev, INPUT_REL_X, sample->rel_x * config->movement_scale, false, K_FOREVER);
		input_report_rel(dev, INPUT_REL_Y, sample->rel_y * config->movement_scale, !config->primary_tap_enabled,
				 K_FOREVER);
		if (config->primary_tap_enabled) {
...
static void pinnacle_poll_work_cb(struct k_work *work)
{
	struct k_work_delayable *delayable = k_work_delayable_from_work(work);
	struct pinnacle_data *drv_data = CONTAINER_OF(delayable, struct pinnacle_data, poll_work);
	const struct pinnacle_config *config = drv_data->dev->config;

	pinnacle_poll(drv_data->dev);
	k_work_reschedule(&drv_data->poll_work, K_MSEC(config->poll_interval_ms));
}

static void pinnacle_init_polling(const struct device *dev)
{
	struct pinnacle_data *drv_data = dev->data;
	const struct pinnacle_config *config = dev->config;

	drv_data->dev = dev;
	k_work_init_delayable(&drv_data->poll_work, pinnacle_poll_work_cb);
	k_work_reschedule(&drv_data->poll_work, K_MSEC(config->poll_interval_ms));
}
...
#define PINNACLE_DEFINE(inst)                                                                      \
    ...
    .movement_scale = DT_INST_PROP(inst, movement_scale),                              \
	};                                                                                         \
